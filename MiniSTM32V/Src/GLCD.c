...
//		LCD_WriteReg(0x0003,(1<<12)|(0<<7)|(1<<5)|(1<<4)|(1<<3) ); 	/* importance */
//#else
...
//#endif
	...
	LCD_WriteReg(0x0001,(0<<10)|(0<<8));     
	LCD_WriteReg(0x0003,(1<<12)|(0<<7)|(0<<5)|(0<<4)|(1<<3) ); 	/* importance */
...
{
	extern const unsigned char _6x13[];
...
* Attention		 : None
*******************************************************************************/
void GUI_TextXY(uint16_t _Xpos, uint16_t _Ypos, uint8_t *str, uint16_t Color, uint16_t bkColor)
{
	Xpos = _Xpos;
	Ypos = _Ypos;
	GUI_Text(str, Color, bkColor);
}
	
void GUI_Text(uint8_t *str,uint16_t Color, uint16_t bkColor)
{
    uint8_t TempChar;
    do
    {
	    ...
	    else
	    {
			...
			if( Xpos < MAX_X - _6X13_WIDTH )
			{
				Xpos += _6X13_WIDTH;
			} 
			else if ( Ypos < MAX_Y - _6X13_HEIGHT*2 )
			{
				Xpos = 0;
				Ypos += _6X13_HEIGHT;
			}   
...
/******************************************************************************
* Function Name  : GUI_Scroll
* Description    : Scroll liens
* Output         : None
* Return         : None
* Attention		 : None
*******************************************************************************/
...
{
	//	LCD_WriteReg(0x0061,0x0001|(1<1)); 
	//	for (int i = 0; i <lines; i++ )
	//		LCD_WriteReg(0x006a,0x0000+i);
	//	
	//	LCD_SetCursor(0,0);
	//	LCD_REG = R34; /* Select GRAM Reg */
	...
	{
		uint16_t buffer[MAX_X];
		buffer[0] = 0;
		LCD_SetCursor(0, MAX_Y - 1 - y);
		for (int x = 0; x < MAX_X; x++)
		{
			LCD_REG = R34; /* Select GRAM Reg */
			uint16_t temp = LCD_RAM; 
			temp = LCD_RAM; 
			buffer[x] = temp;
			LCD_RAM = temp; 
		}
		
		for (int y1 = MAX_Y - 1 - y - lines; y1 >= 0; y1 -= lines)
		{
			LCD_SetCursor(0, y1);
			LCD_REG = R34; /* Select GRAM Reg */
			for (int x = 0; x < MAX_X; x++)
