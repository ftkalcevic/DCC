...
{
    //PORTA = nPortPWMOff;
    static uint16_t nPrevious;
    nSampleHistory[nSamples & 0x03] = nPrevious;
    nSamples++;
    nPrevious = ADCW;
    //byte nSampleEndTime = TCNT0;
    //byte nTime = nSampleEndTime - nSampleStartTime;

    ////if ( nTime * 2 >= 255 - nSampleEndTime )
    //{
    //    nLastSample = nSample;
    //    bLastSample = true;
    //    ADCSRA &= ~(_BV(ADSC));
    ...
    //    if ( nSamples > 16 )
    //    {
    //        if ( nSample > nSampleSum )
    //            nSampleSum = nSample;
    //        if ( nSamples == 20 )
    //        {
    //            //nLastSample = nSampleSum / 8;
    //            nLastSample = nSampleSum;
    //            bLastSample = true;
    //            ADCSRA &= ~(_BV(ADSC));
    //            //PORTA = 0;
    //        }
...
nLastSample = (nSampleHistory[0] + nSampleHistory[1] + nSampleHistory[2] + nSampleHistory[3]) /4;
bLastSample = true;
